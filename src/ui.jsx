...
function Kicker({ children, tone = "red" }) {
  return (
    <span className={`inline-flex items-center text-[12px] font-bold uppercase tracking-[0.32em] text-${tone}`}>
      {children}
    </span>
  );
}
...
function Logo({ onClick, compact }) {
  return (
    <button onClick={onClick} className="group ml-2 flex items-center gap-4 text-left sm:ml-6 lg:ml-10">
      <span className="grid h-28 w-28 shrink-0 place-items-center overflow-visible rounded-full transition-transform group-hover:rotate-[-6deg] -my-6 sm:h-36 sm:w-36 sm:-my-8">
        <img src={IMG.logoMark} alt="Logo Chez Louisa" className="h-full w-full object-contain" />
      </span>
      {!compact && (
        <span className="leading-none">
          <span className="block text-2xl tracking-tight text-red sm:text-3xl" style={{ fontFamily: '"Yeseva One", serif' }}>Chez Louisa</span>
        </span>
      )}
    </button>
  );
