...
function CartePage({ onToast }) {
  ...
  return (
    <div className="pt-44">
      ...
      {/* Filtres collants */}
      <div className="sticky top-[112px] z-30 mt-2 border-b border-line bg-ink/85 backdrop-blur-xl">
        ...
        <div className="space-y-16">
          {sections.map((s) => (
            <div key={s.id}>
              ...
              {/* Photo dessert signature */}
              {s.id === "dolce" && (
                <Reveal className="mt-8 overflow-hidden rounded-2xl ring-2 ring-pink/20">
                  <Img src={IMG.dessert} alt="Dessert signature Louisa" label="Dessert signature Louisa"
                    className="aspect-[16/6] w-full" imgClass="object-cover" />
                </Reveal>
              )}
        ...
        <div className="mt-24">
          <Reveal>
            <div className="text-center">
              <Kicker tone="red">En images</Kicker>
              <h2 className="mt-4 font-display text-4xl font-extrabold tracking-tight text-cream sm:text-5xl">
                La galerie Louisa.
              </h2>
              <p className="mx-auto mt-3 max-w-lg text-cream/70">
                Un aperçu de la maison · les pizzas, les assiettes et la façade rayée de la Rue Colorée.
              </p>
            </div>
          </Reveal>
          <div className="mt-10 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3">
            {[
              { src: IMG.pizzaSlice, label: "Pizza Louisa", tone: "red" },
              { src: IMG.facade, label: "Devanture Louisa", tone: "red" },
              { src: IMG.pizzaBoard, label: "Pizza légumes grillés", tone: "green" },
              { src: IMG.salad, label: "Salade Louisa", tone: "green" },
              { src: IMG.boxes, label: "Boîtes rayées Louisa", tone: "pink" },
              { src: IMG.dessert, label: "Dessert signature", tone: "pink" },
              { src: IMG.pizzaSpicy, label: "Pizza chorizo-basilic", tone: "red" },
            ].map((g, i) => (
              <Reveal key={i} delay={Math.min(i, 6) * 60}>
                <Img src={g.src} alt={g.label} label={g.label}
                  className={`aspect-square w-full overflow-hidden rounded-2xl ring-2 ring-${g.tone}/15`}
