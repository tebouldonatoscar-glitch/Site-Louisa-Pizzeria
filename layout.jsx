  ...
  return (
    <>
      <header className={"fixed inset-x-0 top-0 z-50 transition-all duration-500 " +
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3 sm:px-8 sm:py-3">
          ...
          <div className="mr-2 flex items-center gap-2 sm:mr-6 lg:mr-10">
            <div className="hidden items-center gap-2 sm:flex">
              <Btn href={CONTACT.reservation} target="_blank" tone="red" className="!px-5 !py-2.5 !text-[13px]">
                <IconCalendar size={16} /> Réserver
              </Btn>
      ...
      <div className={"fixed inset-0 z-40 lg:hidden transition-all duration-500 " +
        ...
        <div className="relative flex h-full flex-col overflow-y-auto px-8 pb-12 pt-28">
          ...
          <div className="mt-10 flex flex-col gap-3">
            <Btn href={CONTACT.reservation} target="_blank" variant="solid" tone="red" className="w-full">
              <IconCalendar size={18} /> Réserver une table
            </Btn>
            <Btn href={CONTACT.ubereats} target="_blank" variant="outline" tone="green" className="w-full">
              <IconBike size={18} /> Commander sur Uber Eats
            </Btn>
            <MenuDownloadBtn onToast={onToast} variant="outline" tone="pink" className="w-full" />
          </div>
        </div>
      </div>
    </>
    ...
    <footer className="border-t border-line bg-ink">
      <div className="mx-auto max-w-7xl px-5 py-14 sm:px-8">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5">
              ...
              <li>
                <a href={MENU_PDF_URL} download target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-cream/80 transition-colors hover:text-pink">
                  <IconDownload size={15} /> Menu (PDF)
                </a>
              </li>
              <li>
                <a href={CONTACT.reservation} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-cream/80 transition-colors hover:text-red">
                  <IconCalendar size={15} /> Réserver une table
                </a>
              </li>
              <li>
                <a href={CONTACT.ubereats} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-cream/80 transition-colors hover:text-green">
                  <IconBike size={15} /> Commander (Uber Eats)
                </a>
              </li>
            </ul>
          </div>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-3 border-t border-line pt-6 text-center text-xs text-muted sm:flex-row sm:text-left">
          <span>© {new Date().getFullYear()} Chez Louisa · Fort-de-France, Martinique. · <button onClick={() => go("mentions")} className="underline-offset-2 hover:text-cream hover:underline">Mentions légales</button></span>
...
function MobileActionBar() {
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-ink/95 backdrop-blur-xl shadow-[0_-8px_24px_rgba(0,0,0,0.06)] md:hidden"
      ...
      <div className="mx-auto grid max-w-md grid-cols-4 divide-x divide-line">
        <a href={CONTACT.reservation} target="_blank" rel="noopener noreferrer"
          className="flex flex-col items-center justify-center gap-1 py-3 text-red transition-colors active:bg-red/10">
          <IconCalendar size={20} />
          <span className="text-[11px] font-bold uppercase tracking-wider">Réserver</span>
        </a>
        <a href={CONTACT.ubereats} target="_blank" rel="noopener noreferrer"
          className="flex flex-col items-center justify-center gap-1 py-3 text-green transition-colors active:bg-green/10">
          <IconBike size={20} />
          <span className="text-[11px] font-bold uppercase tracking-wider">Livraison</span>
        </a>
